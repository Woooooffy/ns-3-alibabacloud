// TwoPodRailHostBound, SPLIT-PORTS cabling: the RATE configuration of TE-CCL's 2-pod
// rail-optimized topology with each leaf<->spine0 link built as TWO 25 GBps cables instead
// of one 50 GBps cable (TwoPodRailHostBound.LEAF_SPINE_PORTS = (2, 1)).
// Topology body generated from topology/examples/two_pod_rail_hostbound_splitports.topo via
// topology/main.py; the harness around it (algorithm parse, correctness check, congestion
// monitoring) is two_pod_rail_hostbound.cc's, unchanged.
//
...
//     regswtches 4..5 = spine0 (2 x 25 GBps), spine1 (25 GBps)                      -> TE-CCL 28..29
...
//
//   WHAT THE SPLIT CHANGES. Aggregate capacity is identical (a leaf's uplink is still
//   50 + 25 = 75 GBps), so the schedule in the XML is the same schedule as the unsplit run.
//   What differs is that spine0's half is now two separately queued 25 GBps devices, so the
//   _ports switch JSON has to say WHICH cable each flow takes -- and it does: those entries
//   carry an explicit "port" field, which ParseSwitchJson treats as authoritative. Port
//   numbering is ns-3 device index, laid out below in TE-CCL's port_order:
//
//     leaf(pod p, rail r)  port 0-3   the 4 GPU downlinks, ascending node index
...
//
//   Inputs are the _ports XML/JSON pair; there is no _gated variant of them, so unlike
//   two_pod_rail_hostbound.cc this scenario has no --gate flag.
...
//    LogComponentEnable("CollectivesApplication", LOG_INFO);
    ...
    std::string label = "two_pod_hostbound_ports";
    // Which of the four two_pod_hostbound_*_ports XMLs to run. `rate` picks the rate-annotated
    ...
    // 25 GBps / 700ns: EVERY network port here -- the GPU->leaf rail (the binding cut), both
    // leaf->spine0 cables, and leaf->spine1. Splitting spine0 is what makes the fabric
    // uniformly 25 GBps per port, so a single helper covers all of it.
    ...
    // Rail downlinks: leaf(pod, rail) <- the rail-r GPU of every node in that pod, in
    // ascending node order, so they take leaf ports 0-3.
    ...
    // Leaf-spine mesh: TWO 25 GBps cables to spine0 (leaf ports 4 and 5; spine0 ports 0..7,
    // leaf-major) then ONE to spine1 (leaf port 6; spine1 ports 0..3). Declaration order is
    // what fixes those port numbers, and the _ports switch JSON's "port" fields assume it.
    NetDeviceContainer devs1_32 = link_helper1.Install(regswtches.Get(0), regswtches.Get(4));
    NetDeviceContainer devs1_33 = link_helper1.Install(regswtches.Get(0), regswtches.Get(4));
    NetDeviceContainer devs1_34 = link_helper1.Install(regswtches.Get(0), regswtches.Get(5));
    NetDeviceContainer devs1_35 = link_helper1.Install(regswtches.Get(1), regswtches.Get(4));
    NetDeviceContainer devs1_36 = link_helper1.Install(regswtches.Get(1), regswtches.Get(4));
    NetDeviceContainer devs1_37 = link_helper1.Install(regswtches.Get(1), regswtches.Get(5));
    NetDeviceContainer devs1_38 = link_helper1.Install(regswtches.Get(2), regswtches.Get(4));
    NetDeviceContainer devs1_39 = link_helper1.Install(regswtches.Get(2), regswtches.Get(4));
    NetDeviceContainer devs1_40 = link_helper1.Install(regswtches.Get(2), regswtches.Get(5));
    NetDeviceContainer devs1_41 = link_helper1.Install(regswtches.Get(3), regswtches.Get(4));
    NetDeviceContainer devs1_42 = link_helper1.Install(regswtches.Get(3), regswtches.Get(4));
    NetDeviceContainer devs1_43 = link_helper1.Install(regswtches.Get(3), regswtches.Get(5));
    ...
    const std::string XML_NAME = "two_pod_hostbound_" + coll + (rate ? "" : "_no_rate") + "_fast_epoch_ports.xml";
    ...
    std::string SWITCH_JSON = ns3::SystemPath::Append(ns3::SystemPath::FindSelfDirectory(),
                                                      "../../scratch/json_input/two_pod_hostbound_" + coll + "_fast_epoch_ports.json");
    ...
    const std::string LOG_FILE = ns3::SystemPath::Append(LOG_DIR, "two_pod_hostbound_ports_" + label + ".txt");
